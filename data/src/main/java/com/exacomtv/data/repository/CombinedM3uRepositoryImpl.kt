...
import com.exacomtv.data.local.dao.CombinedM3uProfileDao
import com.exacomtv.data.local.dao.CombinedM3uProfileMemberDao
import com.exacomtv.data.local.dao.ProviderDao
import com.exacomtv.data.local.entity.CombinedM3uProfileEntity
import com.exacomtv.data.local.entity.CombinedM3uProfileMemberEntity
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as com.exacomtv.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != com.exacomtv.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
