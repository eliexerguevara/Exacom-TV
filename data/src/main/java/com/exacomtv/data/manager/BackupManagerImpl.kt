package com.exacomtv.data.manager
...
import com.exacomtv.data.local.dao.EpisodeDao
import com.exacomtv.data.local.dao.FavoriteDao
import com.exacomtv.data.local.dao.MovieDao
import com.exacomtv.data.local.dao.PlaybackHistoryDao
import com.exacomtv.data.local.dao.ProviderDao
import com.exacomtv.data.local.dao.RecordingScheduleDao
import com.exacomtv.data.local.dao.VirtualGroupDao
import com.exacomtv.data.local.entity.ProviderEntity
import com.exacomtv.data.local.entity.RecordingScheduleEntity
import com.exacomtv.data.mapper.toDomain
import com.exacomtv.data.mapper.toEntity
import com.exacomtv.data.preferences.ParentalPinBackupData
import com.exacomtv.data.preferences.PreferencesRepository
import com.exacomtv.data.security.CredentialCrypto
import com.exacomtv.domain.manager.BackupData
import com.exacomtv.domain.manager.BackupConflictStrategy
import com.exacomtv.domain.manager.BackupImportPlan
import com.exacomtv.domain.manager.BackupImportResult
import com.exacomtv.domain.manager.BackupManager
import com.exacomtv.domain.manager.BackupPreview
import com.exacomtv.domain.manager.RecordingScheduleImportDisposition
import com.exacomtv.domain.manager.RecordingScheduleImportOutcome
import com.exacomtv.domain.manager.RecordingScheduleImportSummary
import com.exacomtv.domain.manager.ProtectedCategoryBackup
import com.exacomtv.domain.manager.RecordingManager
import com.exacomtv.domain.manager.ScheduledRecordingBackup
...
import com.exacomtv.domain.repository.CategoryRepository
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.exacomtv.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.exacomtv.domain.model.Result.error("Failed to open output stream")

            com.exacomtv.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.exacomtv.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.exacomtv.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.exacomtv.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.exacomtv.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext com.exacomtv.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.exacomtv.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.exacomtv.domain.model.Result.success(
                BackupImportResult(
            ...
            com.exacomtv.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                com.exacomtv.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        prefs["playerDecoderMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val decoderMode = com.exacomtv.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.exacomtv.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.exacomtv.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = com.exacomtv.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.exacomtv.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.exacomtv.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.exacomtv.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.exacomtv.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.exacomtv.domain.model.RecordingItem.toScheduledRecordingBackup(
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.exacomtv.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.exacomtv.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.exacomtv.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.exacomtv.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.exacomtv.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.exacomtv.domain.model.PlaybackHistory>>() {}.type
