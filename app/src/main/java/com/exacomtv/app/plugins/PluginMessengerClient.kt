...
class PluginMessengerClient @Inject constructor(
    ...
    suspend fun send(
        ...
        val replyMessenger = Messenger(Handler(Looper.getMainLooper()) { message ->
            ...
            if (response.getString(ExacomTVPluginContract.KEY_REQUEST_ID) == requestId &&
        ...
        try {
            withContext(Dispatchers.Main.immediate) {
                val intent = Intent(ExacomTVPluginContract.ACTION_PLUGIN_SERVICE).apply {
            ...
            val request = Message.obtain(null, what).apply {
                ...
                this.data = Bundle(data).apply {
                    putInt(ExacomTVPluginContract.KEY_API_VERSION, ExacomTVPluginContract.API_VERSION)
                    putString(ExacomTVPluginContract.KEY_REQUEST_ID, requestId)
                }
            }
            try {
