package com.exacomtv.app.update
...
private const val GITHUB_RELEASES_LATEST_URL = "https://api.github.com/repos/eliexerguevara/Exacom-TV/releases/latest"
private const val GITHUB_RELEASES_LIST_URL = "https://api.github.com/repos/eliexerguevara/Exacom-TV/releases?per_page=20"
    ...
    suspend fun fetchLatestRelease(): Result<GitHubReleaseInfo> = withContext(Dispatchers.IO) {
        try {
            ...
            val request = Request.Builder()
                ...
                .header("User-Agent", "ExacomTV-Update-Checker")
                .build()
        ...
        for (index in 0 until assets.length()) {
            ...
            when (updateChannel) {
                AppUpdateChannel.Stable -> {
                    if (name.equals("ExacomTV.apk", ignoreCase = true)) {
                        return url
                    }
                    if (fallback == null &&
                ...
                AppUpdateChannel.Beta -> {
                    if (name.equals("ExacomTV-beta.apk", ignoreCase = true)) {
                        return url
                    }
                    if (fallback == null &&
                        name.endsWith(".apk", ignoreCase = true) &&
