...
import com.exacomtv.app.cast.CastConnectionState
import com.exacomtv.app.cast.CastManager
import com.exacomtv.app.cast.CastMediaRequest
import com.exacomtv.app.cast.CastStartResult
import com.exacomtv.app.di.MainPlayerEngine
import com.exacomtv.app.player.LivePreviewHandoffManager
import com.exacomtv.app.plugins.ExacomTVPluginManager
import com.exacomtv.app.util.isPlaybackComplete
import com.exacomtv.app.tv.LauncherRecommendationsManager
import com.exacomtv.app.tv.WatchNextManager
import com.exacomtv.data.remote.stalker.StalkerUrlFactory
import com.exacomtv.data.remote.xtream.XtreamStreamUrlResolver
import com.exacomtv.data.security.CredentialDecryptionException
import com.exacomtv.data.sync.SyncManager
import com.exacomtv.domain.manager.RecordingManager
import com.exacomtv.domain.model.Category
import com.exacomtv.domain.model.ChannelNumberingMode
import com.exacomtv.domain.model.CombinedCategory
import com.exacomtv.domain.model.CombinedM3uProfileMember
import com.exacomtv.domain.model.Program
import com.exacomtv.domain.model.ContentType
import com.exacomtv.domain.model.DecoderMode
import com.exacomtv.domain.model.Episode
import com.exacomtv.domain.model.ExternalPlaybackMode
import com.exacomtv.domain.model.Favorite
import com.exacomtv.domain.model.LiveChannelObservedQuality
import com.exacomtv.domain.model.PlaybackHistory
import com.exacomtv.domain.model.RecordingItem
import com.exacomtv.domain.model.RecordingRecurrence
import com.exacomtv.domain.model.RecordingRequest
import com.exacomtv.domain.model.RecordingStatus
...
import com.exacomtv.domain.model.VirtualCategoryIds
import com.exacomtv.domain.model.VideoFormat
import com.exacomtv.domain.usecase.GetCustomCategories
import com.exacomtv.domain.usecase.MarkAsWatched
import com.exacomtv.domain.usecase.ScheduleRecording
import com.exacomtv.domain.usecase.ScheduleRecordingCommand
import com.exacomtv.domain.repository.ChannelRepository
import com.exacomtv.domain.repository.CombinedM3uRepository
import com.exacomtv.domain.repository.DownloadManager
import com.exacomtv.domain.repository.EpgRepository
import com.exacomtv.domain.repository.MovieRepository
import com.exacomtv.domain.repository.PlaybackHistoryRepository
import com.exacomtv.domain.repository.SeriesRepository
import com.exacomtv.player.Media3PlayerEngine
import com.exacomtv.player.AUDIO_VIDEO_OFFSET_MAX_MS
import com.exacomtv.player.AUDIO_VIDEO_OFFSET_MIN_MS
import com.exacomtv.player.PlaybackState
import com.exacomtv.player.PlayerEngine
import com.exacomtv.player.PlayerError
import com.exacomtv.player.PlayerSubtitleStyle
import com.exacomtv.player.timeshift.LiveTimeshiftBackend
import com.exacomtv.player.timeshift.LiveTimeshiftState
import com.exacomtv.player.timeshift.LiveTimeshiftStatus
import com.exacomtv.player.timeshift.TimeshiftConfig
...
class PlayerViewModel @Inject constructor(
    ...
    internal val favoriteRepository: com.exacomtv.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: com.exacomtv.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: com.exacomtv.data.preferences.PreferencesRepository,
    ...
    internal val pluginManager: ExacomTVPluginManager,
    ...
    internal val currentChannelFlow = MutableStateFlow<com.exacomtv.domain.model.Channel?>(null)
    val currentChannel: StateFlow<com.exacomtv.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<com.exacomtv.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<com.exacomtv.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<com.exacomtv.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<com.exacomtv.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    val remoteShortcutPreferences = preferencesRepository.remoteShortcutPreferences
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000L), com.exacomtv.domain.model.RemoteShortcutPreferences())
    ...
    internal var adoptedHandoffSource: com.exacomtv.app.player.PreviewHandoffSource? = null
    ...
    private var preferredSurfaceMode: com.exacomtv.domain.model.PlayerSurfaceMode =
        com.exacomtv.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<com.exacomtv.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, com.exacomtv.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: com.exacomtv.domain.model.Result<Unit>) {
    ...
    val playerStats: StateFlow<com.exacomtv.player.PlayerStats> =
        activeEngineState(com.exacomtv.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<com.exacomtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<com.exacomtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<com.exacomtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: com.exacomtv.domain.model.StreamInfo,
        requestVersion: Long,
    ...
    ): com.exacomtv.domain.model.StreamInfo? {
        if (!isActivePlaybackSession(requestVersion)) return null
    ...
    private suspend fun probePlaybackUrl(streamInfo: com.exacomtv.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private fun shouldProbePlaybackUrl(
        url: String,
        provider: com.exacomtv.domain.model.Provider
    ): Boolean {
        ...
        if (provider.type != com.exacomtv.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
            provider.type == com.exacomtv.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == com.exacomtv.domain.model.ProviderType.STALKER_PORTAL
            ) &&
                    ...
                    _playerDiagnostics.update {
                        it.copy(
                            ...
                            providerSourceLabel = when (provider.type) {
                                com.exacomtv.domain.model.ProviderType.XTREAM_CODES -> "Xtream Codes"
                                com.exacomtv.domain.model.ProviderType.M3U -> "M3U Playlist"
                                com.exacomtv.domain.model.ProviderType.STALKER_PORTAL -> "Stalker/MAG Portal"
                            }
                        )
                    }
                }
    ...
    internal fun updateChannelDiagnostics(channel: com.exacomtv.domain.model.Channel) {
