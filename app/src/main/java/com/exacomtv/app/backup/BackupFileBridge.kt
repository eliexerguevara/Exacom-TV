package com.exacomtv.app.backup
...
object BackupFileBridge {
    ...
    fun createExportFile(context: Context): File {
        ...
        return File(backupsDir, "exacomtv_backup_${LocalDateTime.now().format(exportNameFormatter)}.json")
            .also { file ->
    ...
    fun copyToImportInbox(context: Context, sourceUri: Uri): Uri? {
        ...
        val targetFile = File(inboxDir, "exacomtv_import_${System.currentTimeMillis()}.json")
        return runCatching {
    ...
    fun buildShareIntent(uri: Uri): Intent {
        ...
        return Intent.createChooser(sendIntent, "Share ExacomTV backup")
            .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
    }
